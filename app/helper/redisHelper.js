const redisHelper = (redisClient) => {
	return {
		...
		hmget: (hash, keys) => hmget(redisClient, hash, keys),
		...
		lpush: (key, value) => lpush(redisClient, key, value),
		lpop: (key) => lpop(redisClient, key),
		ltrim: (key, start, end) => ltrim(redisClient, key, start, end),
		lrange: (key, start = 0, end = -1) => lrange(redisClient, key, start, end),
		publish: (key, value) => publish(redisClient, key, value)
	};
};
...
const hmget = (redisClient, hash, keys) => {
	return new Promise((resolve, reject) => {
		redisClient.hmget(hash, keys, (err, res) => callback(err, res, resolve));
	});
};
...
const lpush = (redisClient, key, value) => {
	return new Promise((resolve, reject) => {
		redisClient.lpush(key, value, (err, res) => callback(err, res, resolve));
	});
};
...
const ltrim = (redisClient, key, start, end) => {
	return new Promise((resolve, reject) => {
		redisClient.ltrim(key, start, end, (err, res) => callback(err, res, resolve));
	});
};

const publish = (redisClient, key, value) => {
	return new Promise((resolve, reject) => {
		redisClient.publish(key, value, (err, res) => callback(err, res, resolve));
	});
};
